...
def string_test(i, hyperparameters, best_fitness, results_dir, runtime):
    s = (
        ...
        "\\caption{Zmiana wartości f. celu - test " + f"{i}" + "}\n"
        "\\label{fitness_" + f"{i}" + "}\n"
        "\\end{figure}\n"
        "\n"
        ...
        "\\caption{Statystyki wartości f. celu - test " + f"{i}" + "}\n"
        "\\label{statics_" + f"{i}" + "}\n"
        "\\end{figure}\n"
        "\n"
...
def string_compare(results_dir, test_num, score, timestamp):
    s = (
        ...
        "\\caption{Rozkład wartości funkcji celu najlepszych osobników}\n"
        ...
        "\\caption{Rozkład czasów działania}\n"
        "\\label{compare_time}\n"
        "\\end{figure}\n"
        "\n"
        "Wykres przedstawiający czasy działania algorytmu, wartości funkcji celu najlepszego osobnika oraz oceny wyników obliczane ze wzoru:\n"
        ...
        "\\caption{Czasy działania, wartości funkcji celu najlepszego osobnika oraz oceny wyników}\n"
        "\\label{compare}\n"
        "\\end{figure}\n"
        "\n"
        "Wykres przedstawiający znormalizowane czasy działania algorytmu, wartości funkcji celu najlepszego osobnika oraz oceny wyników obliczane ze wzoru \\ref{score}:\n"
        "\n"
        "\\begin{figure}[H]\n"
        "\\centering\n"
        "\\includegraphics[width=0.9\\linewidth]{" + f"{results_dir}/comparison_normalized_plot.png" + "}\n"
        "\\caption{Znormalizowane czasy działania, wartości funkcji celu najlepszego osobnika oraz oceny wyników}\n"
...
if __name__ == "__main__":
    ...
    print_section_comparison("20250129_234258", timestamps)
